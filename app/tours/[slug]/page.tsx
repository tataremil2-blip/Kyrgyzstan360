...
const groupTourRoute = [
  { day: "01", route: "Bishkek → Burana → Bokonbaevo", text: "Meet the group, follow the Silk Road to Burana Tower, then travel to the south shore of Issyk-Kul for an eagle-hunting experience and a night by the lake." },
  { day: "02", route: "Bokonbaevo → Song-Kul", text: "Cross Kalmak-Ashuu Pass into the high pastures of Song-Kul. Settle into yurts, meet local herders and watch the light change over the lake." },
  { day: "03", route: "Song-Kul → Naryn → Kel-Suu", text: "A true 4×4 day: mountain passes, Naryn canyon country and the remote Kok-Kiya Valley near Kel-Suu." },
  { day: "04", route: "Kel-Suu → Naryn", text: "Reach Kel-Suu on foot, horseback or a special off-road transfer. Take in the lake, add a boat ride when conditions allow, then return to Naryn." },
  { day: "05", route: "Naryn → Konorchek → Bishkek", text: "Drive back towards Bishkek with a final walk through the sculpted red walls of Konorchek Canyon." },
];
...
export default async function TourDetailPage({ params }: PageProps<"/tours/[slug]">) {
  ...
  return <main className="tour-detail">
    ...
    {slug === "group" && <><section className="group-facts"><span><b>5</b> DAYS</span><span><b>JUN—SEP</b> SEASON</span><span><b>FROM 3</b> GUESTS</span><span><b>4×4</b> JEEP EXPEDITION</span></section><section className="group-why"><div><p>WHY GO TOGETHER</p><h2>More roads.<br /><em>More stories.</em></h2></div><div><p>This is the Kyrgyzstan you want to share: winding passes, yurt dinners, cold lake mornings and the small moments between destinations.</p><ul><li>Travel in a close-knit 4×4 group</li><li>Meet people who came for the same wild places</li><li>Keep the logistics easy — and the adventure real</li></ul></div></section><section className="group-route"><div className="group-route-heading"><p>OUR RECOMMENDED GROUP ROUTE</p><h2>High lakes.<br /><em>One good crew.</em></h2><span>A five-day jeep journey from Bishkek into the high country of Issyk-Kul, Song-Kul and Kel-Suu.</span></div><ol>{groupTourRoute.map((stop) => <li key={stop.day}><span>DAY {stop.day}</span><div><h3>{stop.route}</h3><p>{stop.text}</p></div></li>)}</ol></section><section className="group-inclusions"><div><p>TRANSPARENTLY PLANNED</p><h2>What&apos;s<br /><em>covered.</em></h2></div><div className="group-inclusions-lists"><ul><li className="list-title">INCLUDED</li>{groupIncluded.map((item) => <li key={item}><b>↗</b>{item}</li>)}</ul><ul><li className="list-title">NOT INCLUDED</li>{groupNotIncluded.map((item) => <li key={item}><b>—</b>{item}</li>)}</ul></div></section><section className="group-booking"><p>READY FOR THE ROAD?</p><h2>Join the<br /><em>next crew.</em></h2><span>Tell us your preferred dates and group size. We&apos;ll share availability, the final price and everything you need to know before departure.</span><Link href="/#contact">ASK FOR DATES &amp; PRICE <b>↗</b></Link></section></>}
